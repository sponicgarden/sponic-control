...
const DEVCONTROL_TABS = [
  ...
  },
  {
    tab: "sessions",
    label: "Sessions",
    description: "AI development session history for this project",
    icon: (
      <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor">
  ...
  },
  {
    tab: "planlist",
    label: "PlanList",
    description: "Implementation plans, remediation checklists, and project roadmaps",
    icon: (
      <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor">
    ...
    <div>
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        {DEVCONTROL_TABS.map((item) => (
          <Link
            key={item.tab}
            href={`/${lang}/intranet/devcontrol/${item.tab}`}
            className="flex items-start gap-3 rounded-xl border border-slate-200 p-5 hover:border-slate-400 hover:bg-slate-50 transition-colors group"
          >
