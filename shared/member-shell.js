/**
 * Member Shell - Shared module for all member pages
...
// TAB NAVIGATION
// =============================================
async function renderMemberTabNav(activeTab, authState) {
...
function renderContextSwitcher(authState) {
  ...
  const ADMIN_TAB_MAP = [
    ...
    { perm: 'admin_pai_settings', href: '/members/lifeofpaiadmin.html' },
  ];
...
/**
 * Initialize a member page with auth flow.
 ...
 */
export async function initMemberPage({ activeTab, requiredRole = 'resident', requiredPermission, onReady }) {
  ...
  async function handleAuthState(state) {
    ...
    if (state.appUser && meetsRequirement) {
      ...
      await renderMemberTabNav(activeTab, state);
      ...
      if (onReady && !onReadyCalled) {
        ...
        if (!sessionData?.session) {
          ...
          console.warn('[member-shell] No active session — attempting token refresh');
          const { data: refreshData } = await supabase.auth.refreshSession();
          sessionData = refreshData;
        }
        if (!sessionData?.session) {
          // Refresh also failed — force re-login
          console.warn('[member-shell] Token refresh failed — redirecting to login');
      ...
      if (onReadyCalled) {
        console.warn('[member-shell] Transient auth state change after page authorized — keeping current state');
        return;
      }
